...
import { getTreeStories } from '@/lib/db/story-actions';
...
export default async function StoriesPage({ params }: PageProps) {
  ...
  const stories = await getTreeStories(treeId, { status: 'published' });

  // Get featured stories
  const featuredStories = await getTreeStories(treeId, { status: 'published', featured: true, limit: 3 });
